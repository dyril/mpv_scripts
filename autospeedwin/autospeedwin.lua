...
function findRefreshRate()
    ...
    local floor_fps = math.floor(_global.temp["fps"])
    ...
    local iterator = 1
    if (_global.temp["maxrate"] > floor_fps) then
        iterator = round(_global.temp["maxrate"] / floor_fps)
    elseif (_global.temp["maxrate"] < floor_fps) then
        iterator = round(floor_fps / _global.temp["maxrate"])
    else
    ...
    for rate in string.gmatch(_global.options["rates"], "[%w.]+") do
        ...
        for multiplier = 1, iterator do
            local multiplied_fps = (multiplier * floor_fps)
